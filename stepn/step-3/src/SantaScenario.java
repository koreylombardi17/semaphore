...
public class SantaScenario {
	...
	public static void main(String args[]) {
		...
		for(int day = 1; day < 500; day++) {
			...
			// Check if there are 3 elves in trouble
			if (scenario.elvesWaitList.size() > 2) {
				for (Elf elf : scenario.elvesWaitList) {
					scenario.santa.addElfToDoor(elf);
					elf.setState(Elf.ElfState.AT_SANTAS_DOOR);
				}
				scenario.elvesWaitList.clear();
			}
		}
	}
